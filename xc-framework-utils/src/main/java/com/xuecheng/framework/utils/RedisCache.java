...
/**
 * redis使用工具类
 * Created by WuJianguo on 2017/8/24.
 */
public class RedisCache {
    private volatile static JedisPool pool = null;
    private static RedisCache ru = new RedisCache();

    private RedisCache() {
        if (pool == null) {
            String ip = SysConstants.REDIS_HOST;
            int port = SysConstants.REDIS_PORT;
            int database = SysConstants.REDIS_DATABASE;
            JedisPoolConfig config = new JedisPoolConfig();
            // 控制一个pool可分配多少个jedis实例，通过pool.getResource()来获取；  
            // 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。  
            config.setMaxTotal(SysConstants.REDIS_POOL_MAXACTIVE);
            // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。  
            config.setMaxIdle(SysConstants.REDIS_POOT_MAXIDLE);
            // 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；  
            config.setMaxWaitMillis(SysConstants.REDIS_TIMEOUT);
            config.setTestOnBorrow(true);
            pool = new JedisPool(config, ip, port, 100000,null,database);
        }

    }

    public String[] pipe_get(String... keys) {
        Jedis jedis = null;
        String[] value = null;
        Pipeline pip = null;
        try {
            ...
            pip.multi();
            //批量获取
            if (CheckUtils.isNotEmpty(keys)) {
                for (String key : keys) {
                    pip.get(key);
                }
            }
            Response<List<Object>> response = pip.exec();
            pip.sync();
            List<Object> objects = response.get();
            String[] strings = new String[objects.size()];
            value = objects.toArray(strings);
        } catch (Exception e) {
    ...
    public String[] pipe_set(List<Map<String,String>> list) {
        Jedis jedis = null;
        String[] value = null;
        Pipeline pip = null;
        try {
            ...
            if (CheckUtils.isNotEmpty(list)) {
                for (Map<String,String> key : list) {
                    for(String mkey:key.keySet()){
                        pip.set(mkey, key.get(mkey));
                    }
                }
            }
            Response<List<Object>> response = pip.exec();
    ...
    public Map<String, Map<String, String>> pipe_hgetAll(String... keys) {
        ...
        try {
            ...
            for (int i = 0; i < objects.size(); i++) {
                Map<String, String> map1 = (Map) objects.get(i);
                map.put(keys[i], map1);
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public Map<String, Map<String, String>> pipe_hgetAllForOneM(String[] keys, String[] dateArray) {
        Map<String, Map<String, String>> map = new HashMap<>();
        Jedis jedis = null;
        Pipeline pip = null;
        ...
        try {
            ...
            for (int y = 0; y < objects.size(); y++) {
                List<String> list = (List<String>) objects.get(y);
                Map<String, String> map1 = new LinkedHashMap<>();
                for (int x = 0; x < list.size(); x++) {
                    if (CheckUtils.isNotNull(list.get(x))) {
                        map1.put(dateArray[x], list.get(x));
                    }
                }
                //判断如果redis的mkey缺省，就用0补齐
     ...
     * @return StringBuffer
     */
    private StringBuffer splitAndStringBuffer(String string) {
        StringBuffer sb = new StringBuffer();
        String[] split = string.toString().split(":");
        String dd = DateTimeUtils.nowDateString("dd");
        sb.append(dd);
        for (String s : split) {
            if (s.length() > 0) {
                sb.append("-");
     ...
     * @param key
     * @param value
     * @return 成功 返回更新后的过期时间 失败返回 0
     */
    public Long updateExpire(String key, int value) {
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
    ...
    public Map<String, String> hgetall(String key) {
        ...
        try {
            ...
            LoggerFactory.getLogger(RedisCache.class).error("hgetall", e);
        } finally {
            returnResource(pool, jedis);
        }
        return res;
    ...
    public static RedisCache getRu() {
        return ru;
    }

    public static void setRu(RedisCache ru) {
        RedisCache.ru = ru;
    }
}
